...
It's a part of the Java runtime that allows developers to directly manipulate virtual memory contents.
...
We've been looking at this latest iteration, and how to do stuff with it.
...
You also must provide a layout, to tell the FFM framework how many bits the framework should read, and how it should interpret them.
...
In our example, we use `getAtIndex` multiple times.
There are instances of `get` uses in the code generated by `jextract`.
Similarly, use `set` and `setAtIndex` to set the bits representing your data.
